...
for master_file in master_files:
    ...
    df = pd.read_csv(master_file) #, header=None) # if new csv
    ...
    for match_id, match_df in gb:
        ...
        xml_file = COMMENTARY_DIR + master.replace('_merged.csv.gz', '/').replace('_merged.csv', '/') + xml_name
        if not os.path.isfile(xml_file):
            missing_commentary.add(xml_name)
            continue
        if xml_name in merged_files:
